...
def audio_cb(indata, frames, time, status):
    # фоновий колбек: рахує RMS і трохи згладжує
    global mic_level
    if status:
        return
...
# Тримаємо відкритим адіо-потік, а всередині крутиться гра
with sd.InputStream(samplerate=sr, channels=1, blocksize=block, callback=audio_cb):
    while True:
        ...
        window.blit(score_text, (window_size[0] // 2 - score_text.get_rect().w // 2, 80))
